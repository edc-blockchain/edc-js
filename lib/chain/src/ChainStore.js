...
class ChainStore {
	...
	async getAddressByBlockParams(info) {
		return await Apis.instance().db_api().exec('get_address', info);
	}
	
	getAccountAsync(name_or_id) {
		return new Promise((success, fail) => {
			let result = this.getAccount(name_or_id);
			if(result === undefined) {
				return setTimeout(() => this.getAccountAsync(name_or_id).then(r => success(r)).catch(e => fail(e)), 100);
			}
			success(result);
		});
	}
