...
class ChainStore {
	...
	getAssetBySymbol(symbol) {
		return new Promise((success, fail) => {
			...
			Apis.instance().db_api().exec("lookup_asset_symbols", [[symbol]]).then(asset_objects => {
				...
				this.assets_by_symbol = this.assets_by_symbol.set(symbol, asset_objects[0]);
				
				this.getAssetById(asset_objects[0]).then(r => {
					this.get_asset_by_symbol_promise[symbol].forEach(([s, f]) => s(r));
					return delete this.get_asset_by_symbol_promise[symbol];
				}).catch(e => {
					this.get_asset_by_symbol_promise[symbol].forEach(([s, f]) => f(e));
					return delete this.get_asset_by_symbol_promise[symbol];
				})
			}).catch(error => {
	...
	getAccountAsync(name_or_id, i = 0) {
		return new Promise((success, fail) => {
			if(i > 2)
				return success(undefined);
			let result = this.getAccount(name_or_id);
			if(result === undefined) {
				return setTimeout(() => this.getAccountAsync(name_or_id, i + 1).then(r => success(r)).catch(e => fail(e)), 100);
			}
			success(result);
		});
	}
