...
var {Apis, ChainConfig} = require('edc-ws-js');
...
class TransactionBuilder {
    ...
    finalize(){
        return new Promise((resolve, reject)=> {
            ...
            resolve(Apis.instance().db_api().exec("get_objects", [["2.1.0"]]).then((r) => {
                ...
                // TODO
	            // this.ref_block_prefix = 1252766858;
	            // this.ref_block_num = 16608;
	            // this.expiration = 1496174615;
    ...
    */
    ...
    serialize(){
        console.log(ops.signed_transaction.toObject(this));
...
function _broadcast(was_broadcast_callback){
    return new Promise((resolve, reject)=> {
        ...
        var tr_object = ops.signed_transaction.toObject(this);
        // console.log('... broadcast_transaction_with_callback_new !!!')
        console.log(tr_object);
        Apis.instance().network_api().exec( "broadcast_transaction_with_callback_new", [ function(res) { return resolve(res); } ,tr_object]).then(function(){
            ...
            console.log(message);
            if (error.code == 10) {
                return reject(new Error(error.data.stack[0].format));
            }
            return reject(error)
        });
        return;
