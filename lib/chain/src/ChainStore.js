...
class ChainStore {
	...
	async getFirstAccountIdByKeys(keys, noNotify = false) {
		...
		account_ids.forEach((id, index)=>{
			...
			if(!this.get_account_refs_of_keys_calls.has(keys[index])){
				this.get_account_refs_of_keys_calls = this.get_account_refs_of_keys_calls.add(keys[index]); // for getAccountRefsOfKey (but cache is cleared before it)
			}
