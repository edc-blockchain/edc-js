...
class ChainStore {
	...
	async getFirstAccountIdByKeys(keys) {

		const account = keys.find((key)=>{const accountArray = this.account_ids_by_key.get(key); return accountArray && accountArray.length});
		if(account) return account;

		return await Apis.instance().db_api().exec("get_key_references", [keys]).then(account_ids => {
			account_ids.forEach((id, index)=>{
				let ref = Immutable.Set([id[0]]);
				this.account_ids_by_key = this.account_ids_by_key.set(keys[index], ref)
			})
			this.notifySubscribers();
			return account_ids.find(v=>v && v[0])[0];
			});
