...
class ChainStore {
	...
	getIsExistRequireTransfer(account_id, asset_id = '1.3.1') {
		return new Promise(success => {
			...
			let sum = 0;
			this._getTransfersForMiningPeriod(account_id, account_id).then(operations => {
				operations.forEach(op => {
					sum += op.op.amount.amount;
				});
				sums[asset_id] = [sum, operations[0].id];
	...
	_getTransfersForMiningPeriod(account_id, asset_id = '1.3.1', most = '1.11.0', start = '1.11.0') {
		...
		return new Promise((success, fail) => {
			Apis.instance().history_api().exec("get_account_operation_history2", [
				...
				most[2]--;
				most = most.join('.');
				this._getTransfersForMiningPeriod(account_id, asset_id, most, start).then(nextOperations => {
					success(operations.concat(nextOperations));
				}).catch(err => fail(err));
			}).catch(err => fail(err));
		});
	}
	...
	updateTransactionSum(account_id) {
		...
		Object.keys(sums).forEach(asset_id => {
			let start = sums[asset_id][1].split('.');
			start[2]--;
			start = start.join('.');
			this._getTransfersForMiningPeriod(account_id, asset_id, '1.11.0', start).then(operations => {
				operations.forEach(op => {
					sums[0] += op.op.amount.amount;
					sums[1] = op.id;
				});
			});
		});
