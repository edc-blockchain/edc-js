...
class ChainStore {
	...
	fetchFullAccount(name_or_id) {
		...
		if (!this.fetching_get_full_accounts.has(name_or_id) || Date.now() - this.fetching_get_full_accounts.get(name_or_id) > 5000) {
			...
			Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], true]).then(results => {
				...
				console.log("Error: ", error);
				if (ChainValidation.is_object_id(name_or_id)) this.objects_by_id = this.objects_by_id.delete(name_or_id);else this.accounts_by_name = this.accounts_by_name.delete(name_or_id);
			});
		}
		return undefined;
	}
	...
	_updateObject(object, notify_subscribers, emit = true) {
		...
		// ACCOUNT STATS OBJECT
			...
			if(this.is_actual_history.get(object.owner)) {
				this.is_actual_history = this.is_actual_history.set(this.is_actual_history, false);
			}
			this.updateTransactionSum(object.owner);
		}
		// WITNESS OBJECT
	...
	getIsExistRequireTransfer(account_id, asset_id = '1.3.1') {
		return new Promise(success => {
			if(!this.account_sum_transfers_by_asset.has(account_id)) {
				this.account_sum_transfers_by_asset = this.account_sum_transfers_by_asset.set(account_id, {});
			}
			let sums = this.account_sum_transfers_by_asset.get(account_id);
			if(sums[asset_id] !== undefined) {
				return success(sums[asset_id][0] >= 1000);
			}
			
			let sum = 0;
			this._getTransfersForMiningPeriod(account_id, asset_id).then(result => {
				let {operations, lastID} = result;
				operations.forEach(op => {
					sum += op.op[1].amount.amount;
				});
				sums[asset_id] = [sum, lastID || '1.11.0'];
				this.account_sum_transfers_by_asset = this.account_sum_transfers_by_asset.set(account_id, sums);
				return success(sum >= 1000);
			});
		});
	}
	
	_getTransfersForMiningPeriod(account_id, asset_id = '1.3.1', most = '1.11.0', start = '1.11.0') {
		let startInterval = moment.utc().hours(9).minutes(0);
		if(parseInt(startInterval.format('X')) > parseInt(moment.utc().format('X')))
			startInterval = startInterval.subtract(1, 'day');
		
		return new Promise((success, fail) => {
			Apis.instance().history_api().exec("get_account_operation_history2", [
				account_id,
				start,
				10,
				most,
				...
				let lastID = operations.length ? operations[0]['id'] : null;
				if(operations.length < 10)
					return success({operations, lastID});
				
				let existOldOps = fail;
				let result = operations.filter(op => {
					most = op.id;
					if(parseInt(moment.utc(op.block_time).format('X')) < parseInt(startInterval.format('X'))) {
						existOldOps = true;
						return false;
					}
					return (op.op[1]['amount']['asset_id'] == asset_id);
				});
				...
				most[2]--;
				most = most.join('.');
				this._getTransfersForMiningPeriod(account_id, asset_id, most, start).then(nextResult => {
					success({operations: result.concat(nextResult.operations), lastID});
				}).catch(err => fail(err));
			}).catch(err => fail(err));
		});
	}
	
	updateTransactionSum(account_id) {
		if(!this.account_sum_transfers_by_asset.has(account_id)) {
			return;
		}
		let sums = this.account_sum_transfers_by_asset.get(account_id);
		Object.keys(sums).forEach(asset_id => {
			this._getTransfersForMiningPeriod(account_id, asset_id, '1.11.0', sums[asset_id][1]).then(result => {
				result.operations.forEach(op => {
					sums[asset_id][0] += op.op[1].amount.amount;
					
				});
				let lastID = result.lastID;
				if(!lastID) {
					lastID = sums[asset_id][1];
				}
				sums[asset_id][1] = lastID;
			});
		});
	}
