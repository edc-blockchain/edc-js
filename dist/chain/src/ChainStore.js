...
var _require = require("edinarcoin-ws"),
    Apis = _require.Apis;
...
var ChainStore = function () {
  ...
  */
     ...
     */
				...
				Apis.instance().db_api().exec("get_balance_objects", [[address]]).then(function (balance_objects) {
					...
					for (var i = 0; i < balance_objects.length; ++i) {
						_this8._updateObject(balance_objects[i]);
						set.add(balance_objects[i].id);
					}
					_this8.balance_objects_by_address = _this8.balance_objects_by_address.set(address, Immutable.Set(set));
					_this8.notifySubscribers();
				}, function (error) {
					_this8.balance_objects_by_address = _this8.balance_objects_by_address.delete(address);
				});
			}
			...
			if (result === undefined) {
				...
				Apis.instance().db_api().exec("get_objects", [[id]]).then(function (optional_objects) {
					...
					for (var _i = 0; _i < optional_objects.length; _i++) {
						...
						if (_optional_object) _this9._updateObject(_optional_object, true);else {
   ...
   */
		...
		value: function () {
			var _ref15 = (0, _bluebird.coroutine)(function* (info) {
				var address = yield Apis.instance().db_api().exec('get_address', [info.block, info.n]);
				info.address = address;
				return info;
			});

			function getAddressByBlockParams(_x3) {
		...
		key: "getAccountAsync",
		value: function getAccountAsync(name_or_id) {
			var _this10 = this;

			var i = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;

			return new Promise(function (success, fail) {
				if (i > 2) return success(undefined);
				var result = _this10.getAccount(name_or_id);
				if (result === undefined) {
					return setTimeout(function () {
						return _this10.getAccountAsync(name_or_id, i + 1).then(function (r) {
							return success(r);
						}).catch(function (e) {
							return fail(e);
						});
			...
			if (!this.fetching_by_address.has(address) || Date.now() - this.fetching_by_address.get(address) > 5000) {
				...
				Apis.instance().db_api().exec("get_address_references", [[address]]).then(function (results) {
					if (results && results.length && results[0].length) {
						...
						_this11.accounts_by_address = _this11.accounts_by_address.set(address, accountID);
						var account = _this11.getObject(accountID);
						if (account === null) {
							return null;
						}
						if (account === undefined || account.get('name') === undefined) {
							return _this11.fetchFullAccount(accountID);
						}
						return account;
					}
				});
		...
		value: function getAccountPromise(name_or_id) {
			...
			return new Promise(function (success, fail) {
				var account = _this12.getAccount(name_or_id);
				if (typeof account == 'undefined') {
					return setTimeout(function () {
						_this12.getAccountPromise(name_or_id).then(function (account) {
			...
			if (ChainValidation.is_account_name(id_or_account, true) || id_or_account.substring(0, 4) == "1.2.") {
				...
				if (!_account2) {
					...
					if (_witness_id == undefined) this.fetchWitnessByAccount(_account_id2).then(function (witness) {
						_this13.witness_by_account_id.set(_account_id2, witness ? witness.get('id') : null);
						if (witness && on_update) on_update();
					});
				}
				return null;
				...
				Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], true]).then(function (results) {
					...
					account.orders = account.orders.withMutations(function (set) {
						limit_orders.forEach(function (order) {
							_this17._updateObject(order, false);
							set.add(order.id);
						});
					});
					...
					account.call_orders = account.call_orders.withMutations(function (set) {
						call_orders.forEach(function (co) {
							_this17._updateObject(co, false);
							set.add(co.id);
						});
					});
					...
					account.proposals = account.proposals.withMutations(function (set) {
						proposals.forEach(function (p) {
							_this17._updateObject(p, false);
							set.add(p.id);
						});
					});

					_this17._updateObject(statistics, false);
					var updated_account = _this17._updateObject(account, false);
   ...
   */
      ...
      */
		...
		key: "getObjectAsync",
		value: function () {
			var _ref16 = (0, _bluebird.coroutine)(function* (object_id) {
		...
		key: "getMiningInfo",
		value: function () {
			var _ref17 = (0, _bluebird.coroutine)(function* (account_id) {
		...
		value: function () {
			var _ref18 = (0, _bluebird.coroutine)(function* (account_id) {
				var asset_id = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '1.3.1';
			...
			function getMatureBalance(_x12) {
		...
		value: function () {
			var _ref19 = (0, _bluebird.coroutine)(function* (account_id) {
				var asset_id = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '1.3.1';
			...
			function getIsExistRequireTransfer(_x14) {
		...
		value: function () {
			var _ref20 = (0, _bluebird.coroutine)(function* (account_id, asset_id) {
				var history = (yield this.getTransfers(account_id, asset_id)).toJS();
				var prevMaintenanceMoment = this.getNextMaintenanceMoment().subtract(1, 'day');
				var sum = 0;
			...
			function setSumForMiningPeriod(_x15, _x16) {
		...
		value: function () {
			var _ref21 = (0, _bluebird.coroutine)(function* (account_id) {
				...
				transfers.forEach(function (tx) {
					_this21._addTransferToHistory(account_id, tx, true);
					_this21.setSumForMiningPeriod(account_id, tx.op[1]['amount']['asset_id']);
				});
			});

			function addTransaction(_x17) {
   ...
   */
		...
		key: "getBlock",
		value: function () {
			var _ref23 = (0, _bluebird.coroutine)(function* (height) {
		...
		key: "newBlock",
		value: function () {
			var _ref24 = (0, _bluebird.coroutine)(function* (blockId) {
